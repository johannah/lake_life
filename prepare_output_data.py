...
def get_images(datadir, labeled_data):
    ...
    if not os.path.exists(output_csv):
        ...
        for jpg in jpgs:
            if jpg in labeled_jpgs:
                ...
                output.append([jpg, row['object_annotation_category'].lower(), index])
                ll_cnt +=1
                # reduce search 
                labeled_jpgs.pop(index)
            else:
                ul_cnt +=1
                output.append([jpg, 'unk', 'unk']) 

        outpd = pd.DataFrame(np.array(output), columns=['file', 'label', 'summary_index'])
...
if __name__ == '__main__':
    ...
    all_image_paths = get_images(datadir, labeled_data)
